...
OpenJarvis ships with ten built-in agents across three execution modes (on-demand, scheduled, continuous):
...
CERBERUS is a specialized commercial AI assistant designed for business email management, automated prospection, and approval workflows. It includes:

- **Voice Interaction**: Natural conversation with speech-to-text and text-to-speech
- **Email Management**: Gmail integration with reading, classification, and automated responses
- **Business Rules Engine**: Pricing validation, blocking keywords, and commercial prohibitions
- **Approval System**: Human-in-the-loop validation for sensitive actions
- **Relational Memory**: Contact trust tiers (red/grey/white) and interaction history
- **Autonomous Prospection**: Web-based lead generation with validation
- **Briefing System**: Comprehensive activity summaries and decision journaling
...
1. **Configure Environment Variables**:
   ```bash
   cp .env.example .env
   # Edit .env with your Gemini API key and Gmail OAuth credentials
   ```
...
4. **Start CERBERUS**:
   ```bash
   jarvis serve
   # The server will use cerberus_conversational as the default agent
   ```

5. **Access CERBERUS**:
   - Use the desktop UI or API endpoint with `model: cerberus_conversational`
   - Voice interaction requires STT/TTS backend configuration

### CERBERUS Features

**Business Rules**:
- Automatic pricing validation (Arduino, IA Initiation, IA Professional, IA Dev)
- Blocking keyword detection (juridical terms, urgency, scope extensions)
- Trust tier management (red/grey/white lists)
- Commercial prohibitions enforcement

**Approval Workflow**:
- Actions requiring human validation are queued in ApprovalStore
- Support for approve/reject/edit workflows
- Idempotent execution with comprehensive audit trail
- Decision journaling for compliance

**Email Capabilities**:
- Gmail OAuth integration with modify scope
- Thread-aware email responses
- Automatic classification and routing
- Draft management and validation

**Prospection**:
- Web search-based lead generation
- Anti-hallucination safeguards (source verification)
- Contact deduplication
- First-contact message generation with validation
...
```bash
pytest tests/agents/test_cerberus_e2e.py -v
```

Tests cover:
- Scenario A: Voice conversation with context
- Scenario B: Authorized email auto-send
- Scenario C: Price validation blocking
- Scenario D: Institution tier restrictions
- Scenario E: Urgency keyword detection
- Scenario F: Prospection workflow
- Scenario G: Briefing generation

### CERBERUS Architecture

The system is built around:
- **Rules Engine** (`src/openjarvis/core/cerberus_rules.py`): Business logic validation
- **Approval Store** (`src/openjarvis/tools/approval_store.py`): Action queue and audit trail
- **Relational Memory** (`src/openjarvis/core/cerberus_rules.py`): Contact management
- **Cerberus Agent** (`src/openjarvis/agents/cerberus_conversational.py`): Main conversational agent
- **Prospection Agent** (`src/openjarvis/agents/prospection_agent.py`): Lead generation
- **Briefing Tool** (`src/openjarvis/tools/cerberus_briefing.py`): Activity summaries
