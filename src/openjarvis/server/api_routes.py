...
async def transcribe_speech(request: Request):
    ...
    backend = getattr(request.app.state, "stt_backend", None)
    if backend is None:
        raise HTTPException(status_code=501, detail="STT backend not configured")
...
async def synthesize_speech(req: SynthesizeRequest, request: Request):
    ...
    content_type = content_types.get(result.format, "audio/mpeg")
...
async def speech_health(request: Request):
    ...
    backend = getattr(request.app.state, "stt_backend", None)
    if backend is None:
        return {"available": False, "reason": "No STT backend configured"}
    try:
