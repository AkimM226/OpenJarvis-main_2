...
# ---------------------------------------------------------------------------
# get_tts_backend() discovery function test (Addendum 10)
# ---------------------------------------------------------------------------


def test_get_tts_backend_uses_tts_registry():
    """get_tts_backend() must query TTSRegistry, not SpeechRegistry.

    This is a regression test for the bug in Addendum 10 where
    SpeechRegistry was mistakenly used, causing get_tts_backend() to
    always return None silently.
    """
    ...
    # Register a fake backend into TTSRegistry
    fake_backend = MagicMock()
    fake_backend_class = MagicMock(return_value=fake_backend)
    TTSRegistry.register_value("_test_tts_smoke", fake_backend_class)

    # Ensure it's found
    assert TTSRegistry.contains("_test_tts_smoke")

    # Patch get_tts_backend to check our key comes through
    # (we just verify TTSRegistry is what's consulted — we can't
    # easily test kokoro/cartesia without installing them, so we
    # verify the registry lookup path is correct)
    result = TTSRegistry.get("_test_tts_smoke")()
    assert result is fake_backend, (
        "TTSRegistry.get() must return the registered class; "
        "get_tts_backend() was broken because it queried SpeechRegistry instead."
    )
